...
- 状态：`superseded`
- 取代关系：产品路径由 [`D-002`](../DECISIONS.md#d-002) 承接；公开名称由 [`D-003`](../DECISIONS.md#d-003) 承接；实现语言约束现由 [`D-007`](../DECISIONS.md#d-007) 承接。
...
- 状态：`superseded`
- 取代关系：T-002 已完成；当前模块、进程与语言边界由 [`D-007`](../DECISIONS.md#d-007) 承接。
...
- 日期：2026-08-19
- 原结论：T-002 已证明 Swift 可调用、签名并运行 Apple Virtualization.framework，但这只证明 macOS adapter 的可行实现路径；UI、Core、正式 Provider 和 guest policy agent 的语言与进程边界仍由 `T-003 v3` 结合契约确定。
...
- 原受影响实体与约束：`T-003 v3` 必须显式决定模块、进程与语言边界；在其关闭前不生成正式多语言产品骨架。Swift Probe 代码不提升，只作为调用与 entitlement 证据。
- 状态：`superseded`
- 取代关系：`T-003 v3` 已关闭；当前边界由 [`D-007`](../DECISIONS.md#d-007) 承接。
- 相关提交：D-006 原提交 `46cd9fd`；取代记录位于本条所在 Parent 提交。
